...
public class ExactCover extends DLXSolver {
    ...
    private Map<String, Node> init() {
        ...
        int ec_cnt = 0, box_no=0 ;
        HashMap<String, String> exactCoverMatrix = new HashMap<String, String>(cols);
        String key, value;
           ...
           for(int su_col = 0 ; su_col < grid; su_col ++) {
                for (int su_val = 0; su_val < grid ; su_val++){
                    ...
                    if( (su_row >=0 && su_row < 3) && (su_col >=0 && su_col < 3 ))
                        box_no = 0;
                    if( (su_row >=0 && su_row < 3) && (su_col >=3 && su_col < 6 ))
                        box_no = 1;
                    if( (su_row >=0 && su_row < 3) && (su_col >=6 && su_col < 9 ))
                        box_no = 2;
                    if( (su_row >=3 && su_row < 6) && (su_col >=0 && su_col < 3 ))
                        box_no = 3;
                    if( (su_row >=3 && su_row < 6) && (su_col >=3 && su_col < 6 ))
                        box_no = 4;
                    if( (su_row >=3 && su_row < 6) && (su_col >=6 && su_col < 9 ))
                        box_no = 5;
                    if( (su_row >=6 && su_row < 9) && (su_col >=0 && su_col < 3 ))
                        box_no = 6;
                    if( (su_row >=6 && su_row < 9) && (su_col >=3 && su_col < 6 ))
                        box_no = 7;
                    if( (su_row >=6 && su_row < 9) && (su_col >=6 && su_col < 9 ))
                        box_no = 8;

                    array[constraint_indx + (box_no * grid) + su_val] = Boolean.TRUE;

                    key =  "r"+(su_row+1)+"c"+(su_col+1)+"#"+(su_val+1);

                    StringBuilder builder = new StringBuilder();
                    for (boolean bool: array) {
                        if (bool) builder.append("1"); else builder.append("0");
                    }
        ...
        int row = 1;
         for(String tag : exactCoverMatrix.keySet()){
             String bits = exactCoverMatrix.get(tag);
             int col = 0;
